        ...
        >
          ...
          >
            ...
            <div className="text-center space-y-2">
              <h2 className="text-2xl font-bold text-athlavix-accent">Your Glow Profile</h2>
              <div className="flex justify-center gap-4 mt-4">
                <button 
                  onClick={() => setActiveTab("profile")}
                  className={`px-4 py-2 rounded-full text-xs font-bold transition-all ${activeTab === "profile" ? "bg-athlavix-accent text-white" : "bg-athlavix-accent/5 text-athlavix-accent"}`}
                >
                  Profile Settings
                </button>
                <button 
                  onClick={() => setActiveTab("history")}
                  className={`px-4 py-2 rounded-full text-xs font-bold transition-all ${activeTab === "history" ? "bg-athlavix-accent text-white" : "bg-athlavix-accent/5 text-athlavix-accent"}`}
                >
                  Analysis History
                </button>
              </div>
            ...
            {activeTab === "profile" ? (
                    ...
                    <div className="p-4 bg-athlavix-accent/5 rounded-2xl border border-athlavix-accent/10 space-y-4">
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                          {(["soft", "cheerful", "calm"] as const).map((preset) => (
                            <button
                              key={preset}
                              onClick={() => setVoiceSettings({ ...voiceSettings, preset })}
                              className={`flex-1 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wider transition-all border ${
                                voiceSettings.preset === preset
                                  ? "bg-athlavix-accent text-white border-athlavix-accent"
                                  : "bg-white/50 text-athlavix-accent border-athlavix-accent/10 hover:bg-white/80"
                              }`}
                            >
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between items-center">
                          <p className="text-[10px] font-bold uppercase tracking-widest opacity-50">Speed: {voiceSettings.speed}x</p>
                        </div>
                        <input 
                          type="range" 
                          min="0.5" 
              ...
              <div className="space-y-6">
                {userState.analysisHistory.length > 0 ? (
                  <div className="space-y-4">
                    {userState.analysisHistory.map((analysis) => (
                      <div key={analysis.id} className="p-4 bg-white/50 rounded-2xl border border-athlavix-accent/10 space-y-3">
                        <div className="flex items-center justify-between">
                          <p className="text-[10px] font-bold uppercase tracking-widest opacity-50">
                            {new Date(analysis.timestamp).toLocaleDateString([], { month: 'short', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                          </p>
                          <div className="px-2 py-1 bg-athlavix-accent/10 rounded-full text-[8px] font-bold text-athlavix-accent uppercase tracking-widest">
                            Skin Analysis
                          </div>
                        </div>
                        <div className="flex gap-4">
                          <div className="w-20 h-20 rounded-xl overflow-hidden border border-athlavix-accent/10 shrink-0">
                            <img src={analysis.image} alt="Analysis" className="w-full h-full object-cover" />
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-xs text-athlavix-text line-clamp-4 italic">
                              "{analysis.result}"
                            </p>
                          </div>
                        </div>
                  ...
                  <div className="text-center py-12 space-y-4">
                    <div className="w-16 h-16 bg-athlavix-accent/5 rounded-full flex items-center justify-center mx-auto text-athlavix-accent/20">
                      <Camera size={32} />
                    </div>
                    <p className="text-sm text-athlavix-accent/40 font-medium">No skin analyses yet. Upload a photo in the chat to start! ✨</p>
                  </div>
                )}
              </div>
            )}
