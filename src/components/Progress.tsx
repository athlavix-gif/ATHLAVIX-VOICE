...
import { UserState, LEVELS, AVAILABLE_CHALLENGES } from "../types";
import { Trophy, Award, Star, Zap, Shield, Heart, Sparkles, CheckCircle2, Flame } from "lucide-react";
...
export const Progress: React.FC<ProgressProps> = ({ userState }) => {
  ...
  const challenges = AVAILABLE_CHALLENGES.slice(0, 2); // Show top 2 in sidebar
      ...
      <div className="glass-card p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-full bg-athlavix-accent flex items-center justify-center text-white shadow-inner relative">
              <Trophy size={24} />
              {userState.streak > 0 && (
                <div className="absolute -bottom-1 -right-1 bg-orange-500 text-white px-1.5 py-0.5 rounded-full text-[8px] font-bold flex items-center gap-0.5 shadow-lg border border-white">
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  {isCompleted ? (
                    ...
                    <span className="text-xs font-bold opacity-50">{currentProgress}/{challenge.durationDays}</span>
                  )}
                </div>
